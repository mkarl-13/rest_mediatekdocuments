...
class MyAccessBDD extends AccessBDD {
    ...
    protected function traitementSelect(string $table, ?array $champs): ?array {
        switch ($table) {
            ...
            case "nextidcommande":
                return $this->selectNextIdCommande();
            case "abonnementsfinproche":
                return $this->selectAbonnementsFinProche(); 
           case "" :
    ...
    private function selectCommandesLivresDvd(?array $champs): ?array {
        if (empty($champs)) {
            return null;
        }
        if (!array_key_exists('id', $champs)) {
        ...
        $champNecessaire['id'] = $champs['id'];
        $requete = "SELECT c.id, c.dateCommande, c.montant, cd.nbExemplaire, cd.idLivreDvd, cd.suivi as idSuivi, s.libelle as suivi ";
        $requete .= "FROM document d ";
        $requete .= "JOIN commandedocument cd ON cd.idLivreDvd = d.id ";
        $requete .= "JOIN commande c ON c.id = cd.id ";
        $requete .= "JOIN suivi s ON s.id = cd.suivi ";
    ...
    private function selectCommandesRevue(?array $champs): ?array {
        ...
        $champNecessaire['id'] = $champs['id'];
        $requete = "SELECT r.id, r.periodicite, r.delaiMiseADispo, a.id, a.dateFinAbonnement, a.idRevue, c.id, c.dateCommande, c.montant ";
        $requete .= "FROM revue r ";
        $requete .= "JOIN abonnement a ON a.idRevue = r.id ";
    ...
    private function selectNextIdCommande(): ?array {
        $requete = "SELECT LPAD(IFNULL(MAX(CAST(id AS UNSIGNED)), 0) + 1, 5, '0') AS id FROM commande";
        return $this->conn->queryBDD($requete);
    }

    private function updateCommande(?array $champs): ?int {
        if (empty($champs))
            return null;
        $requete = "UPDATE commandedocument SET suivi = :suivi WHERE id = :id";
    ...
    private function deleteCommande(?array $champs): ?int {
        ...
        try {
            ...
            $this->deleteTuplesOneTable("commandedocument", ["id" => $champs["id"]]);
    ...
    /**
     * Supprime une commande / abonnement dans les tables abonnement et commande
     * @param array|null $champs
     * @return int|null 1 si succès, null si erreur
     */
    private function deleteCommandeRevue(?array $champs): ?int {
        ...
        try {
            ...
            $this->deleteTuplesOneTable("abonnement", ["id" => $champs["id"]]);
            $this->deleteTuplesOneTable("commande", ["id" => $champs["id"]]);
    ...
    /**
     * Retourne les commandes de revues dont l'abonnement se termine dans moins de 30 jours
     * @param array|null $champs
     * @return array|null
     */
    private function selectAbonnementsFinProche(): ?array {
        $requete = "SELECT c.id AS id, c.dateCommande, c.montant, a.dateFinAbonnement, a.idRevue, d.titre ";
